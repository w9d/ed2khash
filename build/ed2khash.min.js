...
d.length&&d.length<=Number.MAX_SAFE_INTEGER&&d.path&&Array.isArray(d.path)}).reduce(function(d,e){return d&&e}))throw"torrent: multi, has invalid files structure";}else{if(2===a)throw"bittorrent2: not tested or complete";throw"bittorrent meta version is unsupported";}}function Qa(a){return a.info.files?a.info.files.map(function(b){return b.length}).reduce(function(b,c){return b+c}):a.info.length};function Sa(){function a(k){function h(){if(r)v=!1,console.error("this should only happen during terminate");else{if(0<O&&0<fa.length&&!W){var l=fa.shift(),B=ha[l];W=!0;n.postMessage({i:l,d:B},[B]);ha[l]=null}X<=k.size&&!D&&6>O?(D=!0,O+=1,g.readAsArrayBuffer(k.slice(X,X+9728E3))):D||W||0!==O||P&&Q?P&&Q&&u():q()}}function q(){D?console.error("this should not happen"):P?Q||(g.onload=function(l){Q=l.target.result;D=!1;h()},D=!0,g.readAsArrayBuffer(k.slice(Math.max(k.size-65536,0),k.size))):(g.onload=
function(l){P=l.target.result;D=!1;h()},D=!0,g.readAsArrayBuffer(k.slice(0,Math.min(65536,k.size))))}function u(){var l={};ia+=k.size;H=H.add(C(k.size,!0));H=H.add(Aa(P));H=H.add(Aa(Q));l.mpc=H.toString(16).padStart(16,"0");1===Y.length?(l.ed2k=e(Y[0]),w(k,l)):(n.onmessage=function(B){l.ed2k=e(B.data.h);w(k,l)},n.postMessage({f:!0,hl:Y}),r=!0)}function w(l,B){f.onfilecomplete&&setTimeout(f.onfilecomplete,1,l,B);c()}function Z(l){if(f.onprogress){var B=t,R=ia;setTimeout(function(){f.onprogress(k,Ba[B]*
...
!1;X+=9728E3;la+=1;h()};Z(-1);h()}function b(k){n||(n=k,n.onerror=function(h){r=!0;console.error("web worker error",h);f.onerror&&setTimeout(f.onerror,1,{message:"Something wrong with HTML5 Web Worker. The error is...\n\n"+h.message})})}function c(){y[++t]?Ea?1!==y.length?(f.onerror&&setTimeout(f.onerror,1,{message:"selected multiple torrent files"}),v=!1):d(y[t]):a(y[t]):(f.onallcomplete&&setTimeout(f.onallcomplete,1),v=!1)}function d(k){var h="";v=!0;33554432<k.size?(f.onerror&&setTimeout(f.onerror,
1,{message:"torrent over arbitrary limit of 32MiB"}),v=!1):(g.onload=function(q){h=String.fromCharCode.apply(null,new Uint8Array(q.target.result));try{var u=new Oa(h)}catch(Z){f.onerror&&setTimeout(f.onerror,1,{message:"torrent_parse says: "+Z});return}q={name:u.name(),size:u.size(),type:"video/mp4",lastModified:Date.now()};var w={ed2k:Ra(u).slice(0,32),mpc:"abcdef0987654321"};40===Ra(u).length&&32===w.ed2k.length&&Ra(u).startsWith(w.ed2k)?(f.onprogress&&setTimeout(f.onprogress,1,q,1,1),f.onfilecomplete&&
setTimeout(f.onfilecomplete,2,q,w),f.onallcomplete&&setTimeout(f.onallcomplete,3),v=!1):f.onerror&&setTimeout(f.onerror,1,{message:"my dumb sanity checks failed"})},g.readAsArrayBuffer(k))}function e(k){var h="0123456789abcdef".split(""),q=new Uint32Array(k);k=q[0];var u=q[1],w=q[2];q=q[3];return h[k>>4&15]+h[k&15]+h[k>>12&15]+h[k>>8&15]+h[k>>20&15]+h[k>>16&15]+h[k>>28&15]+h[k>>24&15]+h[u>>4&15]+h[u&15]+h[u>>12&15]+h[u>>8&15]+h[u>>20&15]+h[u>>16&15]+h[u>>28&15]+h[u>>24&15]+h[w>>4&15]+h[w&15]+h[w>>
12&15]+h[w>>8&15]+h[w>>20&15]+h[w>>16&15]+h[w>>28&15]+h[w>>24&15]+h[q>>4&15]+h[q&15]+h[q>>12&15]+h[q>>8&15]+h[q>>20&15]+h[q>>16&15]+h[q>>28&15]+h[q>>24&15]}var f={onprogress:null,onfilecomplete:null,onallcomplete:null,onerror:null,isbusy:function(){return!0===v},execute:function(k){v?console.error("currently busy processing"):(n||b(new Worker("md4-worker.min.js")),y=k,Fa=y.reduce(function(h,q){return h+q.size},0),Ca=1/Fa,ia=0,Ba=y.map(function(h){return 1/h.size}),t=-1,v=!0,c())},terminate:function(){r=
!0},setworker:b,set_bittorrent_quirk:function(k){v||(Ea=k)}},g=new FileReader,n=null,y=null,t=-1,r=!0,v=!1,Fa=0,Ca=0,ia=0,Ba=[],Ea=!1;g.onerror=function(k){r=!0;console.error("read error",k.target.error);f.onerror&&setTimeout(f.onerror,1,{message:"Something wrong with HTML5 FileReader. The error is...\n\n"+k.target.error})};return f}var T=["ed2khash"],U=ma;T[0]in U||"undefined"==typeof U.execScript||U.execScript("var "+T[0]);
